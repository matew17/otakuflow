...
<template>
  <nav
    ...
    class="fixed bottom-0 left-0 right-0 z-50 border-t border-white/10 flex items-center justify-around pt-2"
    style="background-color: #2a2740; padding-bottom: max(1rem, env(safe-area-inset-bottom))"
  >
    <RouterLink
      ...
      :class="[
        'flex flex-col items-center gap-1 px-4 py-1 transition-transform hover:scale-110 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[--color-accent]',
        isActive(item.to) ? 'text-[--color-accent]' : 'text-white/60',
      ]"
    >
      <Icon :icon="item.icon" class="text-2xl" aria-hidden="true" />
